---
name: github-code-review-with-codegraph
version: 1.0.0
description: >
  Experimental PR review pipeline (codegraph treatment arm) for GitHub;
  indexes the source worktree then posts one validated comment.
disable-model-invocation: true
---

Run `/github-code-review-with-codegraph` on the PR URL in the user message.
Only GitHub URLs whose path contains `/pull/` are in scope.

This skill is the **codegraph treatment** variant of `github-code-review`
(control). Deltas are Stage 2 `codegraph init`, SubAgent gets codegraph access,
and `review-guide-with-codegraph.md`.

# GitHub PR Code Review (Codegraph)
...
validate workspace remote, create/remove worktree, initialize Codegraph on
the worktree, run gates, dispatch SubAgent, post through `gh`.
...
Stage 2 Materialize PR artifacts + create worktree + codegraph init
Stage 3 Security gate on target review rule
Stage 4 Dispatch Codegraph SubAgent for context gathering + code review
...
## Stage 2 - Materialize Artifacts, Worktree, And Codegraph

```text
📥 Stage 2 - Materializing PR artifacts, preparing the worktree, and indexing with Codegraph...
```
...
gh api "repos/${OWNER}/${REPO}/pulls/${PR_NUMBER}/comments" \
  --jq '[.[] | select(.user.type != "Bot") | {author: .user.login, body: .body, path: .path, line: .line, created_at: .created_at}]' \
...
Check if review-rule changed in this PR:

```bash
REVIEW_RULE_CHANGED_IN_PR=$(gh api "repos/${OWNER}/${REPO}/pulls/${PR_NUMBER}/files" \
  --jq '[.[] | .filename] | any(. == "review-rule.md")' 2>/dev/null || echo "false")
```
...
```bash
cd "${WORKSPACE_PATH}" && git fetch origin "${SOURCE_BRANCH}"
cd "${WORKSPACE_PATH}" && git worktree add "${WORKTREE_PATH}" "origin/${SOURCE_BRANCH}" --detach
```

Retry once with `FETCH_HEAD` on failure. Filesystem denial → **hard stop**
`worktree-permission-denied`; else `worktree-failed`.

After the worktree exists, build a fresh Codegraph index **inside the
worktree** (do not reuse the workspace index; do not use `sync` — there is no
prior index in a new worktree):

```bash
cd "${WORKTREE_PATH}" && npx @colbymchenry/codegraph init
```

Init failure → **hard stop** `codegraph-init-failed`. Do not dispatch the
SubAgent.

**Done when:** worktree exists, `.codegraph/` is present under
`WORKTREE_PATH`, and artifact files are written. If `target-review-rule.md`
is empty or missing → **hard stop** `no-review-rule` and show
`references/review-rule-template.md`.
...
```text
🤖 Stage 4 - Running code review in the Codegraph SubAgent...
```
